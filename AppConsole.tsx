...
export default function AppConsole() {
  ...
  const [legacyClaim, setLegacyClaim] = useState<LegacyClaimStatus | null>(null);
  const [legacyClaimConsent, setLegacyClaimConsent] = useState(false);
  ...
  useEffect(() => {
    ...
    getLegacyClaimStatus()
      .then((status) => {
        if (active) setLegacyClaim(status);
      })
      .catch((claimError) => {
        if (!active) return;
        if (
          claimError instanceof ApiError &&
          ["LEGACY_CLAIM_DISABLED", "LEGACY_ACCOUNT_NOT_FOUND"].includes(
            claimError.code,
          )
        ) {
          setLegacyClaim(null);
        ...
        setLegacyClaimError(describe(claimError));
      });
    return () => { active = false; };
  }, [accountReady]);

  async function claimLegacyContext() {
    if (!legacyClaim?.available || !legacyClaimConsent || legacyClaimBusy) return;
    setLegacyClaimBusy(true);
    setLegacyClaimError("");
    try {
      const result = await importLegacyContext("2026-08");
      setLegacyClaim({
        enabled: true,
        available: false,
        status: "LEGACY_CONTEXT_ALREADY_CLAIMED",
      });
      setLegacyClaimConsent(false);
      await refreshThreads();
      setNotice(
        `Contexto anterior importado: ${result.threads} conversa(s), ${result.messages} mensagem(ns) e ${result.attachments} anexo(s).${result.exceptions ? ` ${result.exceptions} item(ns) aguardam revisão.` : ""}`,
      );
    } catch (claimError) {
      setLegacyClaimError(describe(claimError));
    } finally {
  ...
  return (
    <div className="console-shell">
      ...
      <main id="main-content" className="console-main">
        ...
        {accountReady && legacyClaim?.available ? (
          <div className="console-alert console-alert--provisioning" role="region" aria-label="Contexto anterior disponível">
            <div>
              <strong>Encontramos contexto da sua conta anterior.</strong>
              <span>
                Você pode trazer suas conversas e anexos elegíveis para esta nova conta PatroAI. Conversas compartilhadas e itens com pendências de segurança não serão copiados automaticamente.
              </span>
              <label>
                <input
                  type="checkbox"
                  checked={legacyClaimConsent}
                  onChange={(event) => setLegacyClaimConsent(event.target.checked)}
                />{" "}
                Confirmo que desejo importar o contexto elegível da minha conta anterior.
              </label>
              {legacyClaimError ? <span role="alert">{legacyClaimError}</span> : null}
            </div>
            <button
              type="button"
              className="primary-button"
              disabled={!legacyClaimConsent || legacyClaimBusy}
              onClick={() => void claimLegacyContext()}
            >
              {legacyClaimBusy ? "Importando contexto…" : "Importar meu contexto"}
            </button>
          </div>
        ) : null}
        {!configured ? (
